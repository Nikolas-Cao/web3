...
contract RaffleTest is Test {
    ...
    function setUp() external {
        ...
        raffle = new Raffle(0.01 ether, 1 days, subId, address(coordinator), keyHash);
    ...
    function testRevealResultLessThanInterval() external {
        vm.prank(player1.addr);
        ...
        // since the `raffle.revealResult();` revert , so that requestId is invalid
        vm.prank(subOwner.addr);
        vm.expectRevert();
